...
STOP = importdata("./stop/dati1_stop.txt");
...
Fs = 1/Ts ; % la frequenza di campionamento è di 20Hz 
...
rX_ABS = frd(X_ABS,f_abs_g,Ts);
rY_ABS = frd(Y_ABS,f_abs_g,Ts);
rZ_ABS = frd(Z_ABS,f_abs_g,Ts);
...
%% Stabilità Giroscopio stop:
%Introduciamo prima la risposta in frequenza dei tre assi
%e in seguito calcoliamo la funzione di trasferimento:

Ts = 1/Fs;
rX_STOP = frd(X_STOP,f_stop_g,Ts);
rY_STOP = frd(Y_STOP,f_stop_g,Ts);
rZ_STOP = frd(Z_STOP,f_stop_g,Ts);
