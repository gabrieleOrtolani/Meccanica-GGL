...
accX = mydata1(:, 1) - mean(mydata1(:, 1));
accY = mydata1(:, 2) - mean(mydata1(:, 2));
accZ = mydata1(:, 3) - mean(mydata1(:, 3));

Ts = 0.01;
Fs = 1/Ts;

L = max(size(accX)); %numero misurazioni
t = (0:L-1)*Ts;
...
filteredX = filter(b, a, accX);
filteredY = filter(b, a, accY);
filteredZ = filter(b, a, accZ);
...
axis(1) = subplot(3,1,1);
plot(t, filteredX);
title('AccX filtered', 'FontSize', 18);
xlabel('t [sec]', 'FontSize', 16);
ylabel('a(t)', 'FontSize', 16);
grid on;
...
axis(2) = subplot(3,1,2);
plot(t, filteredY);
title('AccY filtered', 'FontSize', 18);
xlabel('t [sec]', 'FontSize', 16);
ylabel('a(t)', 'FontSize', 16);
grid on;
...
axis(3) = subplot(3,1,3);
plot(t, filteredZ);
title('AccZ filtered', 'FontSize', 18);
xlabel('t [sec]', 'FontSize', 16);
ylabel('a(t)', 'FontSize', 16);
grid on;
